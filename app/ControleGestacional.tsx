...
const ControleGestacional = () => {
  ...
  // Agora os retornos condicionais estão depois dos hooks
  if (!user) {
    return (
  ...
  // ...continua com o restante do JSX

  let semanas = 0,
    dias = 0;
  if (controle.idadeGestacionalConsulta) {
    const match = controle.idadeGestacionalConsulta.match(
      /(\d+)\s*semanas?\s*e\s*(\d+)\s*dias?/
    );
    if (match) {
    ...
    <View style={{ flex: 1, backgroundColor: "#FFFAF8" }}>
      ...
      <ScrollView contentContainerStyle={styles.container}>
        ...
        <Text style={[styles.subtitle, styles.alignLeft]}>
          {controle.dataConsulta || "---"} – {semanas || "---"} semanas e{" "}
          {dias || "---"} dias
        </Text>
        ...
        <View
          style={[
            ...
            controle.bebes?.length === 2
              ? styles.babyImageTwo
              : styles.babyImageMultiple,
          ]}
        >
