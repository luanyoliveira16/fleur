import { FontAwesome } from '@expo/vector-icons'; // ícone de calendário
import React, { useState } from "react";
import { Alert, Image, ScrollView, StyleSheet, Text, TextInput, TouchableOpacity, View } from "react-native";
import DateTimePickerModal from "react-native-modal-datetime-picker";
import { createGestante } from "../services/gestanteService"; // serviço de Firebase
...
export default function CadastrarGestante() {
  ...
  const [dataParto, setDataParto] = useState<Date | undefined>(undefined);
  ...
  } else if (formData.tipoGestacao === "única") {
    setBebes([""]);
  }
  ...
  const semanas = Number(formData.semanasGestacao);
    ...
    try {
      ...
      const nomesBebes = tipoGestacaoLower === 'gemelar' ? bebes.filter(b => b.trim() !== "") : [];

      await createGestante(user.uid, {
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      <Image source={require("../assets/images/logoFleur.png")} style={styles.logoAbsolute} />
      ...
      <View style={styles.inputField}>
        ...
        <DateTimePickerModal
            ...
            onConfirm={(date) => { setDataNascimento(date); setShowDatePicker(false); }}
      ...
      <View style={styles.inputField}>
        ...
        <DateTimePickerModal
          ...
          mode="date"
          minimumDate={new Date()} // parto não pode ser no passado
